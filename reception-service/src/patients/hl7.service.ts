...
export class HL7Service {
    ...
    buildHL7v2(payload: {
        ...
        const mshSegment = [
            'MSH', '^~\\&', 'Reception', 'FrontDesk', 'HospitalSystem', 'Main',
            timestamp, '', msgType, messageControlId, 'P', '2.3'
        ].join('|');

        const pidSegment = [
            'PID', '1', '', pidId, '', payload.lastName || '', payload.firstName || '',
            payload.birthDate || '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', payload.action
        ].join('|');
